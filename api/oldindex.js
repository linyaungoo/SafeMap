...
app.post("/save-zone", async(req,res)=>{
  try{
    const { zone_name, township, disaster_type, risk_band, geojson, note } = req.body;
    const geom = normalizeGeoJSON(geojson);

    await pool.query(`
      INSERT INTO zones(zone_name,township,disaster_type,risk_band,geom,note)
      VALUES($1,$2,$3,$4,ST_SetSRID(ST_GeomFromGeoJSON($5),4326),$6)
    `,[zone_name,township,disaster_type,risk_band,JSON.stringify(geom),note||""]);

    res.send("Zone saved");
  }catch(e){
    console.error("ZONE SAVE ERROR:",e.message);
    res.status(500).send("Save failed");
  }
});
...
app.get("/zones", async(req,res)=>{
  const r = await pool.query(`
    SELECT id,zone_name,risk_band,note,ST_AsGeoJSON(geom) geojson FROM zones
  `);
  res.json(r.rows);
});
...
app.put("/zone/:id", async(req,res)=>{
  const { risk_band, note } = req.body;
  await pool.query(`UPDATE zones SET risk_band=$1,note=$2 WHERE id=$3`,
    [risk_band,note||"",req.params.id]);
  res.send("Updated");
});
