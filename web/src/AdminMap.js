...
export default function AdminMap(){
  ...
  const bandColor=b=>b==="SAFE"?"green":b==="WARNING"?"orange":b==="DANGER"?"red":"#7f1d1d";
  ...
  useEffect(()=>{
    if(draftZoneOverlay.current){draftZoneOverlay.current.setMap(null);draftZoneOverlay.current=null;}
    if(pendingZone){
      draftZoneOverlay.current=new window.google.maps.Polygon({
        ...
        fillOpacity:.35,map: map.current
      });
    }
  ...
  async function refresh(){await loadZones();await loadRoutes();applyVisibility();}
    ...
    r.data.forEach(z=>{
      ...
      poly.addListener("click",()=>{
        if(mode===MODES.EDIT_ZONE){
          ...
          setZoneForm({
            name:z.zone_name||"",
            description:(z.note || z.description || ""),
            band:z.risk_band||"SAFE"
          });
          ...
          editZoneOverlay.current=new window.google.maps.Polygon({
            ...
            fillColor:bandColor(z.risk_band),
            strokeColor:bandColor(z.risk_band),
            fillOpacity:.35,editable:true,map:map.current
          });
        }
  ...
  return(
    <>
      <div className="admin-toolbar">
        ...
        <button className="cancel" onClick={()=>setMode(MODES.DEFAULT)}>Cancel</button>
      </div>
      ...
      {(mode===MODES.DRAW_ZONE||mode===MODES.EDIT_ZONE) && (pendingZone||editZoneOverlay.current) &&
        <div className="side-panel open">
          ...
          <select value={zoneForm.band} onChange={e=>setZoneForm({...zoneForm,band:e.target.value})}>
            <option>SAFE</option><option>WARNING</option><option>DANGER</option><option>FORBIDDEN</option>
          </select>
          <div className="color-preview" style={{background:bandColor(zoneForm.band)}}/>
          <div className="panel-actions">
            <button className="save" onClick={mode===MODES.DRAW_ZONE?saveZone:saveEdit}>Save</button>
            <button className="cancel" onClick={()=>setMode(MODES.DEFAULT)}>Cancel</button>
          </div>
        </div>
      }
      ...
      {(mode===MODES.DRAW_ROUTE||mode===MODES.EDIT_ROUTE)&&
        <div className="control-panel">
          <select value={band} onChange={e=>setBand(e.target.value)}>
            <option>SAFE</option><option>WARNING</option><option>DANGER</option>
          </select>
